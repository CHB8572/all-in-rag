"""
milvus.py
功能描述：使用milvus和Visualized-BGE模型构建一个端到端的图文多模态检索引擎
作者：chb
创建时间：2026/6/14
"""
import os
...
from pymilvus import MilvusClient, FieldSchema, CollectionSchema, DataType  # 导入Milvus的Python客户端，用Python代码操作Milvus数据库
import numpy as np
import cv2
...
MILVUS_URI = "http://localhost:19530"  # 编码本地Milvus的默认服务端口19530
...
class Encoder:
    ...
    def __init__(self, model_name: str, model_path: str):
        # 根据传入的模型名称和权重文件路径，创建一个 Visualized-BGE 模型对象
        # 创建好的模型保存到 self.model，后面用它把图片/文本转成向量
    ...
    def encode_query(self, image_path: str, text: str) -> list[float]:
        # 不记录梯度 节省内存并加快推理速度
        with torch.no_grad():
            query_emb = self.model.encode(image=image_path, text=text)  # 二维向量[1,...]
...
def visualize_results(query_image_path: str, retrieved_images: list, img_height: int = 300, img_width: int = 300,
    ...
    panoramic_width = img_width * row_count  # 拼接图的宽度
    ...
    # 把图片转为三维数组  高度 × 宽度 × 颜色通道   ::-1是把颜色通道顺序反转
    query_cv = np.array(query_pil)[:, :, ::-1]  # opencv默认使用的是BGR格式

    resized_query = cv2.resize(query_cv, (img_width, img_height))  # 将查询图片缩放到统一大小
    # 给查询图片四周加 10 像素边框
    # OpenCV 使用 BGR 颜色顺序，所以 (255, 0, 0) 表示蓝色
    bordered_query = cv2.copyMakeBorder(resized_query, 10, 10, 10, 10, cv2.BORDER_CONSTANT, value=(255, 0, 0))
    query_display_area[img_height * (row_count - 1):, :] = cv2.resize(bordered_query, (img_width, img_height))
    # (10, panoramic_height - 20)是文字的坐标，离左边10像素，距离底部20像素
...
# 找到DATA_DIR/dragon下面的所有png格式的文件
